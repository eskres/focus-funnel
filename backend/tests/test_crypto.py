...
def test_round_trip(master_key):
    ...
    assert decrypt_secret(master_key, user_id, secret) == API_KEY


def test_ciphertext_does_not_contain_plaintext(master_key):
    secret = encrypt_secret(master_key, uuid.uuid4(), API_KEY)
...
def test_different_user_id_fails_to_decrypt(master_key):
    ...
    with pytest.raises(DecryptionError):
        decrypt_secret(master_key, uuid.uuid4(), secret)


def test_same_key_encrypts_differently_each_time(master_key):
    user_id = uuid.uuid4()
    first = encrypt_secret(master_key, user_id, API_KEY)
    second = encrypt_secret(master_key, user_id, API_KEY)
...
def test_wrong_master_key_fails(master_key):
    ...
    secret = encrypt_secret(master_key, user_id, API_KEY)
    with pytest.raises(DecryptionError):
        decrypt_secret(os.urandom(32), user_id, secret)
...
def test_tampered_ciphertext_fails(master_key):
    ...
    with pytest.raises(DecryptionError):
        decrypt_secret(master_key, user_id, dataclasses.replace(secret, ciphertext=flipped))
...
def test_unknown_key_version_fails(master_key):
    ...
    secret = encrypt_secret(master_key, user_id, API_KEY)
    with pytest.raises(DecryptionError):
        decrypt_secret(master_key, user_id, dataclasses.replace(secret, key_version=2))
