...
Settings gets a "Your data" section with a button for each, and a dialog for each that says what goes, what stays, and that it cannot be undone. While a deletion runs, the dialog's confirm button shows a spinner on amber, then a tick on green for a moment. Both follow the red delete button's style: the 600 shade at 10% opacity behind, and the 600 shade for the icon. Then the dialog closes for content and usage history, which leave the user on the page with no note, and deleting the usage history tells the usage section to load again. For the account it goes to `/auth/logout`. The section is hidden when `/api/me` reports demo mode.
