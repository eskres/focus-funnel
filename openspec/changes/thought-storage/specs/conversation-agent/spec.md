...
The model SHALL be offered two tools: one that searches the user's thoughts, and one that proposes a thought to file, with a title, a summary, and tags. A tool call SHALL NOT store or change anything by itself: a proposal is stored only when the user confirms it. The result of a tool SHALL be passed back to the model so it can finish its answer, for a bounded number of rounds. The search tool SHALL search the user's own thoughts by meaning and return the matching thoughts' titles, summaries, tags, and dates, or say that none matched. When the search fails, for example for a missing key for the embedding provider, the tool result SHALL say why in plain words and the model SHALL tell the user; the turn SHALL NOT end with an error. Until saving exists, confirming a proposal SHALL report that saving is not available yet and keep the proposal's text.
...
- **THEN** the tool result holds that thought's title, summary, tags, and date
- **AND** the model answers from it

#### Scenario: Search finds nothing

- **WHEN** the search tool runs and no thought matches
...
- **THEN** the tool result says searching needs that provider's key in settings
