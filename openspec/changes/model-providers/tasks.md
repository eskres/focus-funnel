...
- [ ] 8.1 Remove `/api/settings/api-key`, `NEBIUS_BASE_URL`, the `nebius_*` error codes, and the old Nebius key screen and its tests. Check that a search of `backend/` and `frontend/` for `nebius_` finds only the Nebius preset and its config, and that the suites pass
- [ ] 8.2 Run the backend suite on SQLite and on the compose Postgres, and `npm run test`, `npm run lint`, and `npm run build` in `frontend/`. Check that all pass
