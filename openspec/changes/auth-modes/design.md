...
- **Rate limits live in the frontend server,** because the backend sees only the frontend's address. An in-memory token bucket per client address applies `DEMO_RATE_LIMIT` (requests per minute, default 60) to every request and `DEMO_NEW_SESSIONS_PER_HOUR` (default 5) to session starts. The client address is the last `X-Forwarded-For` hop when `TRUSTED_PROXY=true`. Without it, every request shares one bucket, so the limits cap the whole instance. In-memory limits are why demo mode runs one frontend instance, which the demo compose overlay and the docs state.
- **Why no socket address (decided during implementation, 2026-09-24).** Next.js runs `proxy.ts` before it sets `X-Forwarded-For` and gives it no socket address, so a client-sent `X-Forwarded-For` would reach the limiter unchanged. In a container the socket address is rarely the visitor's anyway: behind the HTTPS reverse proxy a public demo needs, it is the proxy's, and on Docker Desktop or rootless Docker it is the bridge gateway. So without `TRUSTED_PROXY=true` the limiter does not guess a per-client address. The docs tell operators to set `TRUSTED_PROXY=true` behind their reverse proxy.
