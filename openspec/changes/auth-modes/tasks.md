...
- [x] 3.1 Add one migration that renames `users.auth0_sub` to `subject`, adds `issuer` (existing rows `auth0-legacy`), `expires_at`, and `demo_notice_accepted_at`, and swaps the unique index to `(issuer, subject)`. Check that `alembic upgrade head` then `downgrade -1` runs cleanly on an empty SQLite database and on the compose Postgres, that an existing row survives as `auth0-legacy`, and that there is a single Alembic head
  - The SQLite check passed: `alembic upgrade head` then `downgrade -1` on an empty database, an existing row survives as `auth0-legacy`, and there is a single Alembic head (`tests/test_auth_modes_migration.py`).
  - Postgres check passed on 2026-09-24 on the compose image (`pgvector/pgvector:0.8.1-pg18-trixie`, Postgres 18.2), empty database: a user stored at `7c2e4f9a1d35` came through the upgrade as `auth0-legacy` with its subject, and the unique constraint became `uq_users_issuer`; `downgrade -1` restored `auth0_sub` and `uq_users_auth0_sub`, kept the legacy user, and removed a user of another issuer; `upgrade head` ran again; there is one head (`5e8b2c0d4a17`), and `alembic check` finds no changes.
...
- [x] 10.3 Write the login setup docs: Pocket ID, another OIDC provider, Firebase, the allow-list, HTTPS for passkeys on a LAN with a Caddy example, and demo mode with its guards. Check by following the Pocket ID section on a clean checkout
  - The docs are in `docs/login.md`, linked from the README. The user followed the Pocket ID section and tested it by 2026-09-24. That test found the sign-out confirmation problem, and `docs/login.md` now says to enter the Logout Callback URL exactly (thought-storage design, decision 13).
...
- [x] 11.5 Start the server with each wrong configuration from task 2.1. Check that it refuses to start and names the setting
  - Passed on 2026-09-24 at `1cb62f3`, each server started with only the listed variables. Backend (`uvicorn`, 8 cases): no `AUTH_MODE`, `AUTH_MODE=ldap`, oidc without `OIDC_ISSUER` or `OIDC_CLIENT_ID`, firebase without `FIREBASE_PROJECT_ID`, and demo with `OIDC_ISSUER`, `FIREBASE_PROJECT_ID`, or `ALLOW_CUSTOM_PROVIDER=true` each exit 3 before startup, naming the setting. Frontend (standalone `node server.js`, 16 cases): the same mode errors, `SESSION_SECRET` missing or short, oidc without `APP_BASE_URL`, `OIDC_ISSUER`, `OIDC_CLIENT_ID`, or `OIDC_CLIENT_SECRET`, an issuer that is not a URL, firebase without each of its three settings, demo with `OIDC_ISSUER`, `FIREBASE_API_KEY`, or `ALLOW_CUSTOM_PROVIDER=true`, and `DEMO_RATE_LIMIT=abc` each exit 1 with `Focus Funnel cannot start:` and the setting. A valid oidc, firebase (frontend only), and demo configuration start on both. Next.js prints `Ready` just before the check exits; no request is served.
