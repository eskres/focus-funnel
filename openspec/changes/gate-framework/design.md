...
Loaded and validated once in the FastAPI lifespan, beside `get_settings()`, into a frozen object. Validation: every registry gate has an entry, no unknown gate ids, all three settings present, values within `limits`. A failure raises at startup, which is what the spec's "server fails to start" scenarios check. The same `limits` block is the range used to validate user overrides, so defaults and overrides can never disagree.

Model ids are **not** validated against Nebius at startup: the backend has no key of its own, keys are per user, and a startup network call to a third party would make the app unbootable during a Nebius outage. A default model that has gone away surfaces as `gate_model_unavailable` on use, with the same message as a bad override.
...
```python
def resolve(gate: Gate, override: GateOverride | None, defaults: GateDefaults) -> ResolvedSettings
```

Returns the three values plus, per setting, `"user"` or `"system"`. The settings endpoint, the test endpoint, and every gate call go through it. There is no second place where a default is applied, which is what keeps "the settings page shows what a call will actually use" true.
...
Saving is refused only on `unsupported`. `unknown` is allowed and the UI marks it unconfirmed, because refusing on `unknown` would make every gate unusable if Nebius ever stopped reporting features. The honest check for `unknown` is the Test action, which for a feature-carrying gate sends a one-tool probe and reports `gate_model_unsupported` when the model rejects the tools parameter or answers without ever being able to call it. That gives the user a real answer instead of a guess.
...
`router` is `routable: False`, so it cannot be picked as a target, and `/router` is not a command (the spec's scenario). Router defaults are deliberately `temperature: 0.0` and a tiny `max_tokens`: the answer is one word, and a hot router is a wrong router.
...
- `components/gate-settings.tsx` renders one card per gate from `GET /api/settings/gates`, joined with `GET /api/models` for the dropdown. A model whose required feature is `unsupported` is rendered disabled with the reason; `unknown` is selectable with an "unconfirmed" note.
...
- [Default model ids in `gates.yaml` are unconfirmed] → Every gate is one dropdown away from working, and `gate_model_unavailable` says exactly that. See the open question; the ids must be confirmed against a real key before the change is applied.
...
2. `gates.yaml` ships with the image. Changing a default model is a config change and a restart, not a migration, and it does not touch users who have overridden that gate.
...
- **Which model id is the default for each gate.** This must be answered before the change is applied; everything else in the plan is independent of the answer. The user's Nebius key is not available here, so the candidates below come from public Nebius Token Factory documentation and provider listings and are **unconfirmed** — the ids a given account can actually use come from `GET /v1/models` with that account's key, and the catalogue changes.
  - `router` (small, fast, cheap; one word of output; tool calling not needed): `Qwen/Qwen3-30B-A3B-Instruct-2507`, `google/gemma-3-27b-it`, or a small Llama instruct variant such as `meta-llama/Meta-Llama-3.1-8B-Instruct-fast`, which is the id Nebius' own function-calling example uses.
  - `push` and `pull` (mid-size instruct models; summarising and answering from retrieved text): `Qwen/Qwen3-235B-A22B-Instruct-2507`, `openai/gpt-oss-120b`, or `zai-org/GLM-5.2`.
  - `explore` (**must** support tool calling): `openai/gpt-oss-120b` and `NousResearch/Hermes-4-405B` are both documented as supporting function calling and tool use; `Qwen/Qwen3-30B-A3B-Instruct-2507` is described as agentic tool use.
  - Suggested starting point, to confirm against a real key: router `Qwen/Qwen3-30B-A3B-Instruct-2507`, push and pull `Qwen/Qwen3-235B-A22B-Instruct-2507`, explore `openai/gpt-oss-120b`.
- **The exact `supported_features` string Nebius uses for tool calling** (`tool_calling`, `function_calling`, `tools`, or another spelling). Read it from one `GET /v1/models?verbose=true` response and fill in the alias set. Until then the feature lands on `unknown`, which the design already handles.
- **Whether `?verbose=true` is available to every account.** If it is not, every model is `unknown` and the Test probe carries the whole feature check. No spec or task changes either way.
