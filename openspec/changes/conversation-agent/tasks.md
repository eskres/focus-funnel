...
- [ ] 1.5 Bring the reusable frontend code over from `backup/gate-build` as listed in `design.md` decision 12, with `gate` removed from names. Check that `npm run test`, `npm run lint`, and `npm run build` pass and that a search of `frontend/` for `gate`, `routing_failed`, and `GateModel` finds nothing
...
- [ ] 2.2 Add the backend error codes `model_not_set` (409), `model_unknown` (400), `model_unsupported` (400), `model_unavailable` (409), `context_full`, `conversation_busy`, `nebius_rate_limited` (429), and `nebius_request_refused` (502) plus the stream-only `output_limit_reached` and `tool_loop_limit`. Check with pytest that each constructor returns its documented status, that `map_nebius_error()` still maps a missing model to `model_unavailable`, that a Nebius 429 maps to `nebius_rate_limited`, and that another unmapped client error maps to `nebius_request_refused` with Nebius's message
...
- [ ] 3.1 Add the `Conversation` and `Message` models from design decision 3, with cascade on user delete and the unique position per conversation. Check with pytest that a duplicate position is rejected, that deleting the user deletes both, and that deleting a conversation deletes its messages
- [ ] 3.2 Add the `UsageEvent`, `ChatModel`, and `UserSettings` models. Check with pytest that a duplicate model per user is rejected, that deleting a conversation sets the usage event's conversation to null and keeps the row, and that deleting the user deletes all three
- [ ] 3.3 Add one Alembic revision that creates the new tables, with `down_revision` `d06a9fe12cba` and no data migration, and reset the local database (it ran the prototype's overrides migration). Check that `alembic upgrade head` then `downgrade -1` runs cleanly on an empty SQLite database and on a fresh compose Postgres, that there is a single Alembic head, and that the app starts on the upgraded database
...
- [ ] 4.3 Add `POST /api/settings/models/test` with a short prompt and a one-tool probe. Check with pytest that a working model reports success with its id, an empty answer reports failure naming the effort, a model rejecting tools gives `model_unsupported`, a missing model gives `model_unavailable`, a refused effort is reported naming the value, a missing key gives `nebius_key_missing`, and no stored setting changes in any case
- [ ] 4.4 Extend `GET /api/models` with each model's per-token prices. Check with pytest against `FakeNebius` that prices and `context_length` are mapped and that missing prices give null
...
- [ ] 6.3 Extend `FakeNebius` with streamed responses: text deltas, tool-call deltas, a final usage chunk, and a mid-stream error. Check with pytest that each shape is delivered as Nebius delivers it
...
- [ ] 8.3 Add the pre-call estimate and refuse with `context_full` when it cannot fit, mapping a Nebius context error to the same code. Check with pytest that no model is called for an estimate over the limit, that the user's message is kept, and that a Nebius context error maps to `context_full`
...
- [ ] 11.3 Show the Nebius key alert with empty choosers when the model list loads without a key. Check with a Vitest test that the alert and its link appear
- [ ] 11.4 Add the usage section with an SVG bar chart of daily spend, a period choice, the split by model, the month total, the estimate label, and the link to the Nebius console for the balance. Check with Vitest that bars render for the mocked days, that an empty state shows, that changing the period refetches, and that no balance is shown
