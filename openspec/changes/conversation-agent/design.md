...
One system prompt is kept in code and not user-editable. It tells the model to talk with the user and help them think, to keep replies short, to use `search_thoughts` for questions about what the user filed before, to use `propose_thought` for something clearly worth keeping or when a discussion reaches a conclusion, to reply in plain text to greetings and unfinished messages, and to ask a short question when unsure. It proposes a stated to-do or idea at once, waits for a conclusion in a discussion, and proposes each conclusion once. The tuned prompt matched the label on 98% and 95% of 43 probe messages over two runs, and no greeting or unfinished message called a tool (`probes/README.md`).

**The three triggers for offering a held proposal are not all model judgment:**

- **Topic change** is the only model judgment. The system note for a turn with a held proposal says: do not repeat it, and first decide whether the latest message is still about its topic. If it is about something else, call `propose_thought` first with the held proposal, updated with anything discussed since, then answer. The note comes after the latest message, and the turn removes it once the model has proposed, so the next round answers instead of proposing again. Probe result (task 12.3): the held proposal was offered on 19 of 20 unrelated messages and on none of 20 related ones, so **topic change stays model judgment and embeddings are not needed**. With the note before the history, it was offered on 4 of 10.
