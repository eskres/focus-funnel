...
When the model proposes a thought, the chat SHALL show the title, summary, and tags in a card the user can edit, with a way to confirm. The user SHALL be able to carry on the conversation without confirming. A confirmed proposal SHALL show its outcome: handed on for saving, or that saving is not available yet.

#### Scenario: Edit and confirm

- **WHEN** a proposal appears and the user edits the summary and confirms
- **THEN** the card shows that the edited thought was handed on, or that saving is not available yet
